...
async function apiResult(endpoint) {
  ...
  try {
    ...
    if (values === null) {
      ...
      return {
        error: [
          {
            Error:
              "No Data To Display For Current Query Insufficient Or Does Not Exist",
          },
        ],
      };
    } else {
    ...
    return {
      error: [
        { Error: "Server Unreachable At Current Time, Please Try Again Later" },
      ],
    };
  }
}
...
async function saveAuthString() {
  ...
  const result = await fetch(
    encodeURI(serverUrl + "/api/check-admin?adminToken=" + authString)
  );
  const resultJson = await result.json();
...
async function adminView() {
  var endpoint =
    "/api/admin-view?adminToken=" +
    localStorage.getItem("aiCompetitionAdminToken");
...
async function adminToken() {
  ...
  host = TEMP_SERVER_URL || "http://localhost:8080";
  var endpoint =
    "/api/generate-admin-token?adminToken=" +
    localStorage.getItem("aiCompetitionAdminToken") +
    "&seed=" +
    seed;
  console.log(host + endpoint);
...
async function changeName() {
  ...
  host = TEMP_SERVER_URL || "http://localhost:8080";
  var endpoint =
    "/api/set-display-name?studentNumber=" +
    studentNumber +
    "&displayName=" +
    newName;
  console.log(host + endpoint);
