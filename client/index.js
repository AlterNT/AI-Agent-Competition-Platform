...
const getPythonPath = async () => {
  try {
    ...
    throw new Error(
      "Python not installed? Please check that `python3` or `python` is in path."
    );
  }
};
...
const compileMaven = async () => {
  try {
    if (process.platform == "win32") {
      let cmd = child_process.spawn("cmd.exe", [
        "/c",
        "mavenCompileScripts\\compileMaven.bat",
      ]);
...
async function main() {
  ...
  switch (language) {
    case "py":
      try {
        fp = child_process.spawn(await getPythonPath(), [
          "-u",
    ...
    case "java":
      ...
      try {
        ...
        fp = child_process.spawn(await getJavaPath(), [
          "-jar",
          "./agents/java/client/target/client-1.0-SNAPSHOT-jar-with-dependencies.jar",
          token,
          game,
          url,
        ]);
