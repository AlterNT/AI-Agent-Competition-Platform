...
public class Action {
     ...
     * @param target the player targetted by this action, or the player in the event
     *               an action has no target
    ...
    private Action(Card card, int player, int target, Card guess) throws IllegalActionException {
        if (player < 0 || player > 3)
            throw new IllegalActionException("Player out of range");
        if (card == null)
            throw new IllegalActionException("Null card specified in action");
     ...
     * 
     * @return the index of the targetted player for 1,2,3,5,6 cards and -1
     *         otherwise
     **/
    public int target() {
        return target;
    ...
    public String toString(String player, String target) {
        ...
        switch (card) {
            case GUARD:
                return str + " and guessed player " + target + " held the " + guess + ".";
            case PRIEST:
                return str + " and asked to see player " + target + "'s card.";
            case BARON:
                return str + " and challenged player " + target + ".";
            case HANDMAID:
                return str + ".";
            case PRINCE:
                return str + " and asked player " + target + " to discard.";
            case KING:
                return str + " and asked player " + target + " to swap cards.";
            case COUNTESS:
                return str + ".";
            case PRINCESS:
                return str + ".";
    ...
    public static Action playGuard(int player, int target, Card guess) throws IllegalActionException {
        ...
        if (guess == null)
            throw new IllegalActionException("Player cannot guess a null card");
        if (guess == Card.GUARD)
            throw new IllegalActionException("Player cannot guess a guard");
