 ...
 * The class comes in two modes, one for the players, which has update
 * operations disabled,
 * and one for the game engine, that can update the state.
 * States of players in the same game will have common data, allowing for an
...
public class State implements Cloneable {
     ...
     * than the target and a not protected,
     * then that player must be targetted instead.
     * f) if all players are protected by the Handmaid and the player a plays a
     * Prince, they must target themselves
     * 
     ...
     * an agent who has just drawn a card.
     * That is
     * a) the player a must hold card c,
     * b) it must be player a's turn
     * c) if the player holds the Countess, they cannot play the Prince or the King
     ...
     * f) if all players are protected by the Handmaid and the player a plays a
     * Prince, they must target themselves
     * There are other rules (such as a player not targetting themselves) that is
     * enforced in the Action class.
     * 
     ...
     * May only be called for non-player states (i.e. the omniscient game engine
     * state)
     * 
     * @param act  the action to be performed
    ...
    public void updateNext(State controller, int[] theNextPlayer) throws IllegalActionException {
        if (controller.statePlayer() != -1) {
            throw new IllegalActionException("operation not permitted in player's state.");
        }
        int temp = theNextPlayer[0] % this.num;
        this.nextPlayer[0] = temp;
    }
}