...
class API {
  ...
  static async init() {
    ...
    app.get("/api/agents", (_, res) => {
      Database.getQueryResult(Database.queryAgents).then((agents) => {
        res.json({ agents });
      });
    });
    ...
    app.get("/api/bots", (_, res) => {
      Database.getQueryResult(Database.queryAgents, { isBot: true }).then(
        (bots) => {
          res.json({ bots });
        }
      );
    ...
    app.get("/api/games", (req, res) => {
      ...
      Database.paginateGames(page).then((result) => {
    ...
    app.get("/api/gamelogs", (req, res) => {
      ...
      Database.paginateGameHistories(page).then((result) => {
        const games = result || databaseDisabledError;
        res.json({ games });
      });
    });
    ...
    app.get("/api/count-game-pages", (_, res) => {
      Database.countPages().then((numPages) => {
    ...
    app.post("/api/set-display-name", (req, res) => {
      ...
      Database.setDisplayName(studentNumber, displayName).then((success) => {
        res.json({ success });
      });
    });
    ...
    app.get("/api/game", (req, res) => {
      ...
      if (gameId && gameId.length == 36) {
        Database.getQueryResult(Database.queryGames, { id: gameId }).then(
          (games) => {
            res.json({ games });
          }
        );
    ...
    app.get("/api/top-winrate", (_, res) => {
      Database.getQueryResult(Database.queryTopWinrate).then((winrate) => {
        res.json({ winrate });
      });
    });
    ...
    app.get("/api/most-improved", (_, res) => {
      Database.getQueryResult(Database.queryMostImproved).then(
        (improvement) => {
    ...
    app.get("/api/check-admin", (req, res) => {
      ...
      if (!adminToken) {
        ...
        Database.authenticateAdmin(adminToken).then((authenticated) => {
      ...
      if (!adminToken) {
        ...
        Database.authenticateAdmin(adminToken).then((authenticated) => {
          if (!authenticated) {
            ...
            Database.getQueryResult(Database.queryAdminView).then((users) => {
              res.json({ users });
            });
          }
        });
    ...
    app.get("/api/admin-get-tournament-winrate", (req, res) => {
      ...
      if (!adminToken) {
        ...
        Database.authenticateAdmin(adminToken).then(async (authenticated) => {
          if (!authenticated) {
            res.json({ users: adminAuthError });
          } else {
      ...
      if (!adminToken) {
        ...
        Database.authenticateAdmin(adminToken).then((authenticated) => {
          if (!authenticated) {
            ...
            Database.generateUserTokens([seed], false).then((tokens) => {
      ...
      if (!adminToken) {
        ...
        Database.authenticateAdmin(adminToken).then((authenticated) => {
          if (!authenticated) {
            ...
            Database.generateUserTokens([seed], true).then((tokens) => {
              const token = tokens?.[0] || tokenAlreadyExistsError;
              res.json({ token });
            });
          }
    ...
    app.get("/api/improvement-rate", (req, res) => {
      const { agentId } = req.query;
      if (agentId) {
        Database.getQueryResult(Database.queryMostImproving, {
          displayName: agentId,
        }).then((improvementArray) => {
          const improvement = improvementArray?.[0] || null;
          res.json({ improvement });
    ...
    return new Promise((resolve) => {
      API.server = app.listen(this.port, () => {
