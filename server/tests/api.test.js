...
console.assert(
  ...
  "This test suite should only be run in the test environment"
);
...
describe("API", () => {
  ...
  describe("Queries With Correct Arguments", () => {
    ...
    it("/api/game", async () => {
      const games = JSON.parse(
        (await request(API.app).get("/api/games").query({ page: 1 })).text
      ).games;
      const response = await request(API.app)
        .get("/api/game")
    ...
    ["winrate", "improvement", "improvement-rate"].forEach((endpoint) => {
      it(`/api/${endpoint}`, async () => {
        const response = await request(API.app)
          .get(`/api/${endpoint}`)
  ...
  describe("Queries With Missing Arguments", () => {
    ["games", "game", "winrate", "improvement", "improvement-rate"].forEach(
      (route) => {
        it(`/api/${route}`, async () => {
