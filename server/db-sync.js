...
export default class DBSync {
  ...
  async start(batchQueries, timeoutDuration) {
    ...
    for (let queryFunction of batchQueries) {
      ...
      this.queryMap.set(queryFunction.name, cachedQuery);
    }
  }
  ...
  getQueryResult(query, filters) {
    ...
    const filterValidation = (key, object, expectation) => {
      ...
      const includesExpectation =
        valueKeys.includes(expectation) || valueArray.includes(expectation);
    ...
    Object.keys(filters).forEach((key) => {
      ...
      result = result.filter((object) =>
        filterValidation(key, object, expectation)
      );
    });
    return result;
  }
