...
class State {
  ...
  newRound() {
    ...
    if (this.player != -1) {
      throw new Error("Operation not permitted in player's state.");
    }
  ...
  playerState(player) {
    ...
    if (player < 0 || this.num <= player) {
      throw new Error("Player out of range.");
    }
    try {
  ...
  isLegalAction(a, t, c, drawn) {
    ...
    if (
      (this.hand[a].name === "Countess" || drawn.name === "Countess") &&
      (c.name === "King" || c.name === "Prince")
    ) {
    ...
    if (t != -1) {
      ...
      if (
        this.playerHandmaid(t) &&
        (!this.allHandmaid(a) || c.name === "Prince")
      ) {
  ...
  guardAction(a, t, guess) {
    if (this.allHandmaid(a)) {
      ...
      return `\nPlayer ${this.name(
        t
      )} had the ${guess} and is eliminated from the round.\n`;
    } else {
  ...
  baronAction(a, t) {
    ...
    if (elim != -1) {
      ...
      this.hand[elim] = null;
      for (let i = 0; i < this.num; i++) {
        this.known[i][elim] = true;
      }
      return `\nPlayer ${this.name(elim)} holds the lesser card: ${
        this.discards[elim][this.discardCount[elim] - 1].name
      }, and is eliminated.\n`;
    }
    this.known[a][t] = true;
    this.known[t][a] = true;
  ...
  princeAction(t) {
    ...
    if (discarded.name === "Princess") {
      ...
      return `\nPlayer ${this.name(
        t
      )} discarded the Princess and is eliminated.\n`;
    }
  ...
  princessAction(a) {
    ...
    let outcome = `\nPlayer ${this.name(
      a
    )} played the Princess and is eliminated.\n`;
    outcome += `\n Player ${this.name(a)} was also holding the ${
      this.discards[a][this.discardCount[a] - 1]
    }.\n`;
    return outcome;
  }
  ...
  allHandmaid(player) {
    ...
    for (let i = 0; i < this.num; i++) {
      noAction =
        noAction && (this.eliminated(i) || this.handmaid[i] || i === player);
    }
    return noAction;
  }
  ...
  unseenCards() {
    ...
    let aCount = 0;
    for (let j = 0; j < this.num; j++) {
      if (!this.eliminated(j)) {
        rem[aCount++] = this.hand[j];
      }
    }
    for (let p = 0; p < this.deckSize(); p++) {
      rem[alive + i] = this.deck[this.top[0] + i];
    }
    rem.sort();
    return rem;
  }
  ...
  roundWinner() {
    ...
    for (let p = 0; p < this.num; p++) {
      if (!this.eliminated(p)) {
        let dv = 0;
        for (let j = 0; j < this.discardCount[p]; j++) {
          dv += this.discards[p][j].value;
        }
        if (
          this.hand[p].value > topCard ||
          (this.hand[p].value == topCard && dv > discardValue)
        ) {
          winner = p;
  ...
  gameWinner() {
    let threshold =
      this.num == 4 ? 4 : this.num == 3 ? 5 : this.num == 2 ? 7 : 0;
    for (let i = 0; i < this.num; i++) {
