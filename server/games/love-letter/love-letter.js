    ...
    try {
      while (!gameState.gameOver()) {
        ...
        while (!gameState.roundOver()) {
          ...
          console.log(
            `Player ${gameState.getNextPlayer()} draws the ${
              topCard.name
            } card.`
          );
          ...
          try {
            ...
            this.randomAgent.newRound(
              gameState.playerState(gameState.getNextPlayer())
            );
          ...
          for (let i = 0; i < numPlayers; i++) {
            this.agents[i].see(act, playerStates[i]);
          }
        }
        console.log(
      ...
      for (let i = 0; i < numPlayers; i++) {
        scoreboard[i] = gameState.score(i);
      }
      return scoreboard;
    } catch (e) {
      process.error(e);
