...
export default class Action {
  ...
  constructor(card, player, target, guess) {
    ...
    if (card == null) {
      throw new Error("Null card specified in action.");
    }
  ...
  makeString(player, target) {
    ...
    switch (this.card) {
      case Card.GUARD:
        return str + ` and guessed player ${target} held the ${this.guess}.`;
      case Card.PRIEST:
  ...
  static playGuard(player, target, guess) {
    ...
    if (guess == null) {
      throw new Error("Player cannot guess a null card.");
    }
    if (guess == "GUARD") {
      throw new Error("Player cannot guess a guard.");
    }
